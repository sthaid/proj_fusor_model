...
int main(int argc, char **argv)
{
    ...
    printf("T  = %lg  KELVIN\n", T);
    printf("n  = %lg  MOLES\n", n);
    printf("N  = %lg  NUM_D2   %lg NUM_D2/CC\n", N,  N / M3_TO_C3(V));
    printf("N  = %lg  NUM_D2   %lg NUM_D2/CU_M\n", N,  N / V);
    
    printf("\n------------------------\n\n");

    printf("ELECTRONS_PER_COULOMB = %lg\n", ELECTRONS_PER_COULOMB);
    exit(0);
...
void electron_model(void)
{
    ...
    for (t = 0; t < MODEL_DURATION; t += dt) {
        ...
        for (me = active_head.lh_first; me != NULL; me = me_next) {
            ...
            ionization_occurred = ((double)random()/RAND_MAX <= fraction);
            if (ionization_occurred) {
                num_ionized_me++;
                printf("IONIZATION  t=%lg  active=%ld  created=%ld  ionized=%ld  fract=%f  left_chamber=%ld  "
                       "excess=%ld   current_leaving_chamber=%lg\n",
                    t, 
                    num_active_me, 
                    num_created_me, 
                    num_ionized_me, 
                    ...
                    (num_left_chamber_me * electrons_per_modelelectron) / t * COULOMB_PER_ELECTRON);
            ...
            // endif
            if (ionization_occurred) {
                modelelectron_t * new_me = free_head.lh_first;
                assert(new_me);
                LIST_REMOVE(new_me, entries);
                LIST_INSERT_HEAD(&active_head, new_me, entries);
                new_me->v = 0;
                new_me->r = me->r;
                num_active_me++;
                //num_created_me++;

                me->v = 0;  // XXX more todo here
            }
        }
    }
    ...
    if (!new_tbl_initialized) {
        ...
        for (ev = 0; ev < MAX_EV; ev++) {
            ...
            if (ev < tbl[0].ev) {
                new_tbl[ev] = 0;
                continue;
            } else if (ev > tbl[MAX_TBL-1].ev) {
                new_tbl[ev] = tbl[MAX_TBL-1].cs*1e-20;
                continue;
            }
